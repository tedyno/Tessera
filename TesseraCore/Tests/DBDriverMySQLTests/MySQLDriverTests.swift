...
/// Integration tests against a live MySQL instance. Skipped unless the
/// environment provides connection info.
///
/// Set: TESSERA_MYSQL_HOST, TESSERA_MYSQL_PORT, TESSERA_MYSQL_USER,
/// TESSERA_MYSQL_PASSWORD, TESSERA_MYSQL_DB
final class MySQLDriverTests: XCTestCase {
    ...
    private func makeProfileOrSkip() throws -> (ConnectionProfile, Secrets, NetworkEndpoint) {
        ...
        guard let host = env["TESSERA_MYSQL_HOST"],
              let user = env["TESSERA_MYSQL_USER"],
              let db = env["TESSERA_MYSQL_DB"] else {
            throw XCTSkip("Set TESSERA_MYSQL_* env vars to run MySQL integration tests")
        }
        let port = Int(env["TESSERA_MYSQL_PORT"] ?? "3306") ?? 3306
        let profile = ConnectionProfile(
            name: "test", kind: .mysql, host: host, port: port,
            database: db, username: user, tlsMode: .disable)
        let secrets = Secrets(databasePassword: env["TESSERA_MYSQL_PASSWORD"])
